...
function simDataset!{T<:Number}(p::Vector{T}, bdata::Array{T,2},cdata::Array{T,2};noise::T=5.0)
...
function fittransit{T<:Number,T2<:Number}(bdata::Array{T,2},cdata::Array{T,2},p::Vector{T2})
  @assert(length(p)==10)
...
function gradtest!{T<:Number,T2<:Number}(p::Vector{T},gstore::Vector{T},bdata::Array{T2,2},cdata::Array{T2,2},jconfig)
  tb=bdata[:,1]
  tc=cdata[:,1]
  dtb=bdata[:,2]
  dtc=cdata[:,2]
...
function hesstest!{T<:Number}(p::Vector{T}, hstore::Array{T,2}, bdata::Array{T,2}, cdata::Array{T,2}, jconfig)
  tb=bdata[:,1]
  tc=cdata[:,1]
  dtb=bdata[:,2]
  dtc=cdata[:,2]
 ...
 # testhess=new_minihess(fsingle,p, [timeb[3]],'b')
  ...
  for k in 1:length(p)
    for j in 1:length(p)
      ...
      for i2 in 1:length(tc)
          d2chisqc+= 2*dTTVcdp[i2,j]*dTTVcdp[i2,k]/ec[i2]^2 +2*(ttvc[i2]-dtc[i2])*d2TTVc[j,k,i2]/ec[i2]^2
      end
