...
function plogprior(z::Vector{Float64})
  ...
  #tblogprior= -0.5*log(2*pi*dti^2)-(param[3]-ti_b)^2 /(2*dti^2)
  #tclogprior= -0.5*log(2*pi*dti^2)-(param[8]-ti_c)^2 /(2*dti^2)
  tblogprior=0.0
...
function pgradlogtarget(z::Vector{Float64})
  ...
  #gradlogprior[3]+= -(param[3]-ti_b)/dti^2
  #gradlogprior[8]+= -(param[8]-ti_c)/dti^2
  ...
  gradtest!(param,gstore,bData,cData,jconfig)
  ...
  return (gradlogprior+gradloglikelihood)*B
end
...
function ptensorlogtarget(z::Vector{Float64})
  ...
  hesstest!(param,hstore,bData,cData,jconfig)
