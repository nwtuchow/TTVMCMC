...
function gradtransit!{T<:Number,T2<:Number}(p::Vector{T},gstore::Vector{T},bdata::Array{T2,2},cdata::Array{T2,2},jconfig; jmax=jmax0)
  ...
  function fb2{Tp<:Number}(timeb::Vector{Tp})  #gives TTVb array
    planet1=TTVFaster.Planet_plane_hk(p[1],p[2],p[3],p[4],p[5])
    planet2=TTVFaster.Planet_plane_hk(p[6],p[7],p[8],p[9],p[10])
    altttvb=Array(Tp,length(tb))
  # alternative arrays as workspace
  ...
  function fc2{Tp<:Number}(timec::Vector{Tp})
    planet1=TTVFaster.Planet_plane_hk(p[1],p[2],p[3],p[4],p[5])
    planet2=TTVFaster.Planet_plane_hk(p[6],p[7],p[8],p[9],p[10])
    altttvc=Array(Tp,length(tc))
    # alternative arrays as workspace
  ...
  for j in 1:length(p)
    ...
    for i in 1:length(tb)
      dchsqb+= 2*(timeb[i]+ttv1[i]-tb[i])*(dTTVbdp[i,j] + dtimeb[i,j]) /eb[i]^2
    end

    for i2 in 1:length(tc)
      dchsqc+= 2*(timec[i2]+ttv2[i2]-tc[i2])*(dTTVcdp[i2,j] + dtimec[i2,j]) /ec[i2]^2
    end
...
function vhess{T<:Number}(fsingle,x::Vector{T},times::Vector{T}, planet::Char)
    l=length(x)
...
function hesstransit!{T<:Number}(p::Vector{T}, hstore::Array{T,2}, bdata::Array{T,2}, cdata::Array{T,2}, jconfig; jmax=jmax0)
 ...
 function fb{Tp<:Number}(p::Vector{Tp})  #gives TTVb array
  ...
  d2TTVb=vhess(fsingle,p,timeb,'b')
  d2TTVc=vhess(fsingle,p,timec,'c')
  ...
  for k in 1:length(p)
    for j in 1:length(p)
      ...
      d2chisqc=0.0
      for i in 1:length(tb)
          d2chisqb+= 2*(dTTVbdp[i,j]+dtimeb[i,j])*(dTTVbdp[i,k]+dtimeb[i,k])/eb[i]^2 +2*(timeb[i]+ttvb[i]-tb[i])*d2TTVb[j,k,i]/eb[i]^2
      end

      for i2 in 1:length(tc)
          d2chisqc+= 2*(dTTVcdp[i2,j]+dtimeb[i2,j])*(dTTVcdp[i2,k]+dtimeb[i2,k])/ec[i2]^2 +2*(timec[i2]+ttvc[i2]-tc[i2])*d2TTVc[j,k,i2]/ec[i2]^2
      end
